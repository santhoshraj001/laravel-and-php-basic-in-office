...
<?php
// AJAX request handler (no page refresh) - enhanced
if (isset($_GET['ajax_sort'])) {

    // connect (same as rest of file)
    ...
    // sort param
    $sort = (isset($_GET['ajax_sort']) && $_GET['ajax_sort'] === "desc") ? "DESC" : "ASC";
    ...
    // filter_dates support (optional, same format you use: start,end)
    $where = "";
    if (!empty($_GET['filter_dates'])) {
    ...
    // fetch rows (keep same columns as your table)
    $sql = "SELECT * FROM students $where ORDER BY id $sort LIMIT $start, $limit";
    $result = mysqli_query($conn, $sql);

    while ($row = mysqli_fetch_assoc($result)) {
        $photos = json_decode($row['photo'], true) ?: [];
        // build html for photos column (similar to main page)
        $photosHtml = '';
        if ($photos) {
            foreach ($photos as $p) {
                $path = "uploads/" . htmlspecialchars($p);
                if (file_exists(__DIR__ . "/uploads/" . $p)) {
                    $photosHtml .= "<img src='{$path}' class='table-thumb' alt='' />";
                } else {
                    $photosHtml .= "<span style='color:#c33'>(missing)</span>";
                }
            }
        } else {
            $photosHtml = "<span style='color:#666;'>(none)</span>";
        }

        // echo a row (use intval/htmlspecialchars for safety)
        echo "<tr>
...
/* (The rest of your original file remains unchanged — I will paste it exactly as you provided, with two small safe edits:
   1) the <tbody> that previously had no id is now <tbody id="tableBody"> so AJAX can replace it.
   2) the ID header arrow uses a button with type="button" to prevent any accidental form submit.)
*/
...
.table-thumb { width: 40px; height: 40px; object-fit: cover; margin-right:4px; border-radius:4px; }
.popup-img { width:80px; height:80px; object-fit:cover; margin-right:6px; border-radius:4px; }
.sort-button { background: none; border: none; padding: 0; font-size: 14px; cursor: pointer; }
</style>
</head>
<body>
        ...
        <table class="data-table" id="studentTable">
            <thead>
                <tr>
                    <th>Select</th>
                    <?php
                        // initial sort direction (server-side) - default desc as page query used ORDER BY id DESC
                        $initSort = (isset($_GET['sort']) && $_GET['sort'] === "desc") ? "desc" : "asc";
                        $arrowChar = $initSort === "asc" ? "▲" : "▼";
                    ?>
                    <th>ID 
                        <button type="button" id="sortArrow" class="sort-button" aria-label="Toggle sort" onclick="sortTable()"><?= $arrowChar ?></button>
                    </th>
                    <th>Name</th>
                    <th>Email</th>
                    <th>Phone</th>
...
<script>
/* ====== Calendar & popup logic (unchanged) ====== */
...
<!-- ====== AJAX SORT SCRIPT ====== -->
<script>
// Determine initial sort and read current filters from URL or hidden input
let currentSort = "<?= (isset($_GET['sort']) && $_GET['sort'] === 'desc') ? 'desc' : 'asc' ?>";

// Read filter_dates from the hidden input if present (server renders it into the page)
function getFilterDates() {
    const el = document.getElementById('selectedDates');
    if (el && el.value) return el.value;
    // fallback to URL param
    const params = new URLSearchParams(window.location.search);
    return params.get('filter_dates') || '';
}
...
function sortTable() {
    ...
    // Update arrow UI (button)
    const arrow = document.getElementById('sortArrow');
    if (arrow) arrow.innerText = (currentSort === 'asc') ? '▲' : '▼';

    // Build AJAX URL
    let url = window.location.pathname + '?ajax_sort=' + encodeURIComponent(currentSort);

    const filterDates = getFilterDates();
    if (filterDates) {
        url += '&filter_dates=' + encodeURIComponent(filterDates);
    }

    // keep current page (or send 1 if you prefer)
    url += '&page=' + encodeURIComponent(getCurrentPage());
    url += '&limit=10';

    // Optionally show a loading row
    ...
    fetch(url, { credentials: 'same-origin' })
        ...
        .catch(err => {
            console.error('AJAX sort error', err);
            if (tbody) tbody.innerHTML = '<tr><td colspan="11" style="text-align:center;color:#c33">Unable to load data</td></tr>';
        });
}
</script>
